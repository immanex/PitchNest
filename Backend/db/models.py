...
class User(Base):
    ...
    startups: Mapped[list["StartupProfile"]] = relationship(back_populates="founder", cascade="all, delete-orphan")
    pitches: Mapped[list["Pitch"]] = relationship(back_populates="user", cascade="all, delete-orphan")
...
class StartupProfile(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=generate_uuid)
    founder_id: Mapped[str] = mapped_column(String, ForeignKey("users.id", ondelete="CASCADE"))
    ...
    pitches: Mapped[list["Pitch"]] = relationship(back_populates="startup", cascade="all, delete-orphan")
...
class Pitch(Base):
    ...
    startup_id: Mapped[str | None] = mapped_column(String, ForeignKey("startup_profiles.id", ondelete="SET NULL"), nullable=True)
    ...
    recommendations: Mapped[list["AIRecommendation"]] = relationship(back_populates="pitch", cascade="all, delete-orphan")

class AIRecommendation(Base):
    __tablename__ = "ai_recommendations"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=generate_uuid)
    pitch_id: Mapped[str] = mapped_column(String, ForeignKey("pitches.id", ondelete="CASCADE"))
